...
const AddHealthRecord = ({ isOpen, onClose, onSubmit, selectedAnimal, isLoading }) => {
  ...
  // Load animals if not pre-selected
  useEffect(() => {
    if (!selectedAnimal && isOpen) {
      loadAnimals();
    }
  }, [isOpen, selectedAnimal]);

  // Set animal ID when selectedAnimal changes
  useEffect(() => {
    if (selectedAnimal) {
      setValue('animalId', selectedAnimal._id);
    }
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Add Health Record" size="lg">
      <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
        ...
        {selectedAnimal && (
          <div className="bg-emerald-50 rounded-lg p-4 flex items-center gap-3">
            ...
            <div>
              <div className="font-medium text-emerald-900">{selectedAnimal.name}</div>
              <div className="text-sm text-emerald-700">{selectedAnimal.species} • {selectedAnimal.breed}</div>
            </div>
          </div>
        )}
